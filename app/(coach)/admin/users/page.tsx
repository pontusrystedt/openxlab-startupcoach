...
export default function AdminUsersPage() {
  ...
  const [editForm, setEditForm] = useState({ name: "", email: "", role: "", startupId: "" })
  ...
  useEffect(() => {
    Promise.all([
      ...
      setUsers(u)
      setStartups(s.map((x: { id: string; name: string }) => ({ id: x.id, name: x.name })))
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/org/users/${editTarget.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        role: editForm.role,
        startupId: editForm.startupId || null,
      }),
    })
    const data = await res.json()
    setEditSaving(false)
    ...
    if (res.ok) {
      ...
      setEditError(data.error ?? "Kunde inte spara")
    }
  }
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {form.role === "ENTREPRENEUR" && (
              <div>
                ...
                <select required value={form.startupId}
      ...
      {editTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Redigera konto</h2>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={editForm.role}
                    onChange={(e) => setEditForm((f) => ({ ...f, role: e.target.value, startupId: "" }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]">
                    <option value="COACH">Coach</option>
                    <option value="CLIENT_ADMIN">Admin</option>
                    <option value="ENTREPRENEUR">Grundare</option>
                ...
                {editForm.role === "ENTREPRENEUR" && (
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Bolag</label>
                    <select value={editForm.startupId}
                      onChange={(e) => setEditForm((f) => ({ ...f, startupId: e.target.value }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            ...
            <p className="text-sm text-gray-600 mb-1">
              Vill du verkligen radera <strong>{deleteTarget.name ?? deleteTarget.email}</strong>?
            </p>
